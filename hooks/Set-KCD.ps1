...
Function Get-Delegation()
{
    try
    {
        ...
        If ($AdObject -ne $null)
        {
            Write-Output `n"Computer name $TrustedComputer is trusted for delegation for the following service(s) to computer(s):"
            Write-Output "--------------------------------------------------------------------------------------------"`n
        ...
        {
            Write-Output `n"No delegation has been configured for computer $TrustedComputer."`n
        }
    }
    Catch
    {
        Write-Output `n"An error occurred searching for Computer name $TrustedComputer in Active Directory!"`n
    }
}
...
switch($PSCmdlet.ParameterSetName)
{
    {($_ -eq "Add") -or ($_ -eq "Replace") -or ($_ -eq "Remove")}
    {
        ...
        {
            Write-Output "`nThe Object for computer $TrustedComputer could not be found in Active Directory.`n"
        }
    }
    ...
    "Import"
    {
        If ($File -ne $null)
        {
            ...
            Catch
            {
                Write-Output `n"$File file not found!`n"
                exit
            }
            ...
            foreach($Item in $Records)
            {
                ...
                Write-Output "--------------------------------------------------------------------------------------------"`n
                Write-Output "Added $ServiceType/$TrustingComputer,$ServiceType/$TrustingComputer.$env:UserDnsDomain to $TrustedComputer."`n
            }
        }
    }
